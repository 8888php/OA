...
// 所长、财务副所长、财务科长、科研科室主任、科研副所长 显示所有项目
        //如果是茹爱玲,显示所有的党证总站,科研项目,汇总报销,id=44; admin也要看到 id=1 是admin
        if ($this->is_who() != false || in_array($this->userInfo->id, array(44, 1))) {
            return true;
        }
    ...
    public function sub_declares() {
        ...
        if($_POST['is_calculation'] == 1){
            ...
            if($_POST['projectname'] != 0 ){
                $checkcost = $this->check_subject_cost_submit($_POST['projectname'], $_POST['subject']);
                if($checkcost['code'] == -1){
...
//其他审批人 暂时不处理
    ...
    public function report_form($pid = 0, $export = false) {
        ...
        $this->set('is_pro', $is_pro);
        //如果是茹爱玲,显示所有的党证总站,科研项目,汇总报销,id=44; admin也要看到 id=1 是admin
        if (empty($pid) || (!($this->is_who() || in_array($this->userInfo->id, array(44, 1))))) {
            // 不属于五个职务的 请求用户
            if ($is_pro == false) {
...
//                $saveArr['year'] = $this->request->data('year');
    ...
    public function ajax_step3_edit() { 
        ...
        if ($this->request->is('ajax') && $this->request->data('upstep') == 'step3') {
            ...
            $saveArr['travel'] = 0;
            !empty($_POST['travel']) && $saveArr['travel'] = $_POST['travel'];
            $saveArr['international'] = 0;
            !empty($_POST['international']) && $saveArr['international'] = $_POST['international'];
            $saveArr['cooperation'] = 0;
            !empty($_POST['cooperation']) && $saveArr['cooperation'] = $_POST['cooperation'];
            $saveArr['labour'] = 0;
            !empty($_POST['labour']) && $saveArr['labour'] = $_POST['labour'];
            $saveArr['consult'] = 0;
            !empty($_POST['consult']) && $saveArr['consult'] = $_POST['consult'];
            $saveArr['indirect_manage'] = 0;
            !empty($_POST['indirect_manage']) && $saveArr['indirect_manage'] = $_POST['indirect_manage'];
            $saveArr['indirect_performance'] = 0;
            !empty($_POST['indirect_performance']) && $saveArr['indirect_performance'] = $_POST['indirect_performance'];
            $saveArr['indirect_other'] = 0;
            !empty($_POST['indirect_other']) && $saveArr['indirect_other'] = $_POST['indirect_other'];
            $saveArr['other'] = 0;
            !empty($_POST['other']) && $saveArr['other'] = $_POST['other'];
            $saveArr['other2'] = 0;
            ...
            $saveArr['other3'] = 0;
            !empty($_POST['other3']) && $saveArr['other3'] = $_POST['other3'];
            $saveArr['activitiesfee'] = 0;
            !empty($_POST['activitiesfee']) && $saveArr['activitiesfee'] = $_POST['activitiesfee'];
            $saveArr['servicefee'] = 0;
            !empty($_POST['servicefee']) && $saveArr['servicefee'] = $_POST['servicefee'];
            $saveArr['subsidy'] = 0;
            !empty($_POST['subsidy']) && $saveArr['subsidy'] = $_POST['subsidy'];
            $saveArr['total'] = array_sum($saveArr);  // 总额
