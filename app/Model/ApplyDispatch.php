...
class ApplyDispatch extends AppModel {
    ...
    private function get_shengpin_arr ($type = 2) {
        return Configure::read('approval_process')['apply_dispatch'][$type];
    }
    ...
    private function dep_approve($main_arr, $user_info, $status) {
        ...
        //获取请假天数
        $sql_qingjia = "select *from t_apply_dispatch where id='{$main_arr[0]['t_apply_main']['attr_id']}'";
    ...
    private function get_by_pos_dep($pos_id, $dep_id, $team_id=0) {
        ...
        if ($pos_id == 15) {
            ...
            //发文员
            $sql_25 = "select *from t_user where position_id='{$pos_id}' and del=0";
            $arr_25 = $this->query($sql_25);
            $ret_arr[$this->next_uid] = empty($arr_25[0]['t_user']['id']) ? 0 : $arr_25[0]['t_user']['id'];
            $ret_arr[$this->next_id] = $pos_id;
        } elseif ($pos_id == 20) {
