...
class ApplyRequestReport extends AppModel {
    ...
    public function apply_approve($main_id, $user_info, $status) {
        ...
        $chuchai_sql = "select * from t_apply_request_report where id='{$main_arr[0]['t_apply_main']['attr_id']}'";
        $chuchai_arr = $this->query($chuchai_sql);
        $data = array();
        $data['sum_day'] = $chuchai_arr[0]['t_apply_request_report']['days'];
        $data['dep_pro'] = $chuchai_arr[0]['t_apply_request_report']['project_id'];
        $data['dep_id'] = $chuchai_arr[0]['t_apply_request_report']['department_id'];
        if ($type == 2) {
            //部门
